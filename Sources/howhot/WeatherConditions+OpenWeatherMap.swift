...
extension WeatherConditions {
    init(from owm: Container, isMetric: Bool = false) {
        ...
        self.clouds = owm.clouds.flatMap { PercentReading(reading: Float($0.all)) }
        self.rain = owm.rain.flatMap { RatePerHour.measured(reading: $0.rate, isMetric: isMetric) }
        self.snow = owm.snow.flatMap { RatePerHour.measured(reading: $0.rate, isMetric: isMetric) }
    }
}
