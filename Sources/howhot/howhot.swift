...
struct AppOptions: ParsableArguments {
    ...
    @Flag(help: "Metric units instead of imperial") var metric = false
...
@main struct howhot: AsyncParsableCommand {
    ...
    mutating func run() async throws {
        ...
        guard let geoKey = config.string(forKey: "geokey") else { throw AppError.missingApiKey("geokey") }
        guard let weatherKey = config.string(forKey: "openweathermap") else {
        ...
        do {
            if appOptions.boringOutput {
                ...
                consolLines.forEach { line in terminal.output(line, newLine: true) }
            }
        } catch {
...
extension WeatherConditions {
    static func createZoneMap(configReader: ConfigReader) -> WeatherConditions.ZoneMap {
        ...
        return keysAndValues.isEmpty
            ? WeatherConditions.TempComfortZone.defaultZoneMap : Dictionary(uniqueKeysWithValues: keysAndValues)
    }
}
